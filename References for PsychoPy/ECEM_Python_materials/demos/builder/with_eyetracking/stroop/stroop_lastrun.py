...
# -*- coding: utf-8 -*-
"""
This experiment was created using PsychoPy2 Experiment Builder (v1.78.00), Mon  5 Aug 21:00:08 2013
...
expInfo = {u'Eye Tracker': u'SMI_iview_std.yaml', u'Participant': u''}
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath=u'/Users/jwp/Dropbox/ecem/materials/demos/builder/with_eyetracking/stroop/stroop.psyexp',
    savePickle=True, saveWideText=True,
    dataFileName=filename)

# Setup the Window
win = visual.Window(size=[1920, 1080], fullscr=False, screen=0, allowGUI=True, allowStencil=False,
    monitor='testMonitor', color='black', colorSpace='rgb', units='pix')
...
instrText = visual.TextStim(win=win, ori=0, name='instrText',
    ...
    units='pix', pos=[0, 0], height=50, wrapWidth=800,
...
word = visual.TextStim(win=win, ori=0, name='word',
    ...
    pos=[0, 0], height=100, wrapWidth=None,
...
gaze_cursor = visual.GratingStim(win=win, name='gaze_cursor',units='pix', 
    tex='sin', mask='circle',
...
trials = data.TrialHandler(nReps=5.0, method=u'random', 
    extraInfo=expInfo, originPath=u'/Users/jwp/Dropbox/ecem/materials/demos/builder/with_eyetracking/stroop/stroop.psyexp',
...
for thisTrial in trials:
    ...
    frameN = -1
    # update component parameters for each repeat
    word.setColor(stimColor, colorSpace=u'rgb')
    word.setText(stimText)
    ...
    trialComponents.append(gaze_cursor)
    ...
    while continueRoutine:
        ...
        if t >= 0.0 and gaze_cursor.status == NOT_STARTED:
            # keep track of start time/frame for later
            gaze_cursor.tStart = t  # underestimates by a little under one frame
            gaze_cursor.frameNStart = frameN  # exact frame index
            gaze_cursor.setAutoDraw(True)
        if gaze_cursor.status == STARTED:  # only update if being drawn
